...
export const getEditPost = async (req, res) => {
  ...
  if (!post) {
    return res
      .status(404)
      .render("notfound", { pageTitle: "포스트를 찾을 수 없음" });
  }
...
export const deletePost = async (req, res) => {
  ...
  for (let i = 0; i < comments.length; i++) {
    ...
    commentOwner.comments.splice(
      commentOwner.comments.indexOf(comments[i]._id),
      1
    );
    commentOwner.update();
  }
  ...
  for (let i = 0; i < bookmarks.length; i++) {
    ...
    bookmarkOwner.bookmarks.splice(
      bookmarkOwner.bookmarks.indexOf(bookmarks[i]._id),
      1
    );
...
export const deleteBookmark = async (req, res) => {
  ...
  bookmarkOwner.bookmarks.splice(
    bookmarkOwner.bookmarks.indexOf(bookmark[0]._id),
    1
  );
  bookmarkOwner.save();
